...
void USART_IRQHandler(void)
{
	...
	if (USART_GetITStatus(USART_PORT, USART_IT_RXNE) != RESET)
	{
		...
		if (len_in_buf < USART_BUFFER_SIZE)		//判断接收缓冲区剩余长度
		{
			ch = (uint8_t)USART_ReceiveData(USART_PORT);	//读取一字节数据
			RdBuffer.data[RdBuffer.idx_in++] = ch;				//保存到接收缓冲区
			if ((ch == 0)
